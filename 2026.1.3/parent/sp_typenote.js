IE.fModule({
  f: function (exports, require, module, global) {
    ...
    const TradTypeNote = ObjetTraduction_1.TraductionsModule.getModule(
      'TypeNote',
      {
        Annotation: {
          Absent: '',
    ...
    class TypeNote {
          ...
          switch (aArrayGenresNote[x]) {
            case Enumere_Annotation_1.EGenreAnnotation.absent:
              lCaractereAnnotation.Absent =
                TradTypeNote.CaractereAnnotation.Absent;
              break;
            ...
            case Enumere_Annotation_1.EGenreAnnotation.inapte:
              lCaractereAnnotation.Inapte =
                TradTypeNote.CaractereAnnotation.Inapte;
              break;
      ...
      static validerNote(aValeur, aOptions = {}) {
        ...
        const lMessage = TypeNote.messageValidation(lOptions);
        if (
          !lNote ||
      ...
      static messageValidation(aOptions = {}) {
        const lDefaultOptions = {
          avecVirgule: true,
          afficherAvecVirgule: true,
          min: 0,
        ...
        const lResult = {
          message: '',
          noteMin: new TypeNote(lValeurMin),
          noteMax: new TypeNote(lValeurMax),
        };
        if (lOptions.messageMinMax) {
          lResult.message = ObjetChaine_1.GChaine.format(
            lOptions.messageMinMax,
            [
              lOptions.afficherAvecVirgule
                ? lResult.noteMin.getNote()
                : lResult.noteMin.getNoteSansDecimaleForcee(),
              lOptions.afficherAvecVirgule
                ? lResult.noteMax.getNote()
                : lResult.noteMax.getNoteSansDecimaleForcee(),
            ],
          );
        }
        return lResult;
