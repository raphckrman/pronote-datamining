IE.fModule({
  f: function (exports, require, module, global) {
    ...
    require('@cp/Produit/Css/IEHtml.Bouton.css');
    ...
    const Bouton = (aProps) => {
      return IE.jsx.str(
        'div',
        Object.assign({ [IEHtml_1.IEHtml.attrJsxComp]: 'Bouton' }, aProps),
        aProps.children,
      );
    };
    exports.Bouton = Bouton;
    const u_Dots = '...';
    IEHtml_1.IEHtml.addJsxComponent('Bouton', (aContexteCourant, aOutils) => {
      let lOptions = {
        disabled: false,
        theme: Type_ThemeBouton_1.TypeThemeBouton.secondaire,
        tailleBordure: undefined,
        fontWeight: '',
        hasDots: false,
      };
      ...
      const lImage = aContexteCourant.node.getAttribute('ie_image') || '';
      if (lImage) {
        aContexteCourant.node.removeAttribute('ie_image');
      }
      const lImageIconSvg = aContexteCourant.node.getAttribute('svg') || '';
      if (lImageIconSvg) {
        aContexteCourant.node.removeAttribute('svg');
      }
      const lIconSize = aContexteCourant.node.getAttribute('svgsize') || '';
      if (lIconSize) {
        aContexteCourant.node.removeAttribute('svgsize');
      }
        ...
        if (lInfosEvent.estFonction) {
          ...
          lJBouton
            ...
            .on({
              focusin: function () {
                if (
                  ObjetNavigateur_1.Navigateur &&
                  ObjetNavigateur_1.Navigateur.ShiftTouche !== null
                ) {
                  lKeyUpValiderAIgnorer = _estToucheValider(
                    ObjetNavigateur_1.Navigateur.CodeTouche,
                  );
                  if (lKeyUpValiderAIgnorer) {
                    $(window)
      ...
      let lClassDefaut = '';
      if (!lBouton.style.height && !lImage && !lImageIconSvg) {
      ...
      let lWrapperHtml;
      if (lImage || lImageIconSvg) {
        lWrapperHtml = lEstBoutonCarre
          ? (aHtml) => {
              return IE.jsx.str(
                'span',
                {
                  class: Divers_css_1.SD.ellipsisMultilignes,
                  style: { '--clamp-number': 2 },
                },
                aHtml,
              );
            }
        ...
        const lHtmlImage = lImage
          ...
          : lImageIconSvg
            ? IE.jsx.str(
                'span',
                {
                  class: IEHtml_Bouton_css_1.SIEHtmlBouton.ieBoutonIcon,
                  style: lIconSize ? { fontSize: lIconSize } : false,
                },
                lImageIconSvg,
              )
            : IE.jsx.str('i', {
          ...
          lInnerHtml = IE.jsx.str(
            ...
            IE.jsx.str(
              ...
              IE.jsx.str(
                ...
                {
                  ie_ellipsis:
                    aContexteCourant.node.hasAttribute('ie_ellipsis'),
                },
                lInnerHtml,
              ),
            ),
      ...
      lClasse += IEHtml_Ripple_css_1.SIEHtmlRipple.ieRipple + ' ';
      ...
      aOutils.addCommentaireDebug(
        lRacine,
        'Bouton' +
          (lImage ? ' ie_image="' + lImage + '"' : '') +
          (lIconSize ? ' svgsize="' + lIconSize + '"' : ''),
      );
