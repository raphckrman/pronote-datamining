  ...
  f: function (exports, require, module, global) {
    ...
    const TypeGenreRenduTAFUtil = {
      estGenreValable(aGenre) {
        const lApplicationPN = (0, AccessApp_1.getApp)();
        const lParametresPN = lApplicationPN.getObjetParametres();
        let lResult = true;
        if (
          lParametresPN &&
      ...
      getLibelle(aGenre) {
        switch (aGenre) {
          ...
          case TypeGenreRenduTAF.GRTAF_RenduPronoteEnregistrementAudio:
            return 'Audio - A enregistrer sur l'Espace Élèves (3 minutes max)';
          default:
        }
        return '';
      },
      ...
      getLibelleDeposer(aGenre, aPourEleve = false) {
        ...
        switch (aGenre) {
          ...
          case TypeGenreRenduTAF.GRTAF_RenduPronote:
            return aPourEleve
              ? lApplicationSco.getEtatUtilisateur().pourPrimaire()
                ? 'Déposer la copie'
                : 'Déposer ma copie'
              : 'A rendre en ligne';
          case TypeGenreRenduTAF.GRTAF_RenduKiosque:
