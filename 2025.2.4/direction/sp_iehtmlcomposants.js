  ...
  f: function (exports, require, module, global) {
    ...
    IEHtml.addClass(
      ...
      (aContexteCourant, aNodeName) => {
        if (
          !aContexteCourant.node.hasAttribute('role') &&
          aNodeName !== 'article' &&
          !aContexteCourant.node.classList.contains('tiny-view-sans-label')
        ) {
          aContexteCourant.node.setAttribute('role', 'group');
          if (!aContexteCourant.node.hasAttribute('aria-label')) {
            aContexteCourant.node.setAttribute(
              'aria-label',
              GlossaireWAI_1.TradGlossaireWAI.TinyView,
            );
            aContexteCourant.node.setAttribute(
              Tooltip_1.Tooltip.attrType,
              Tooltip_1.Tooltip.Type.default,
